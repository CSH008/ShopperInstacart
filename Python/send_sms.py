...
def send_sms(phone, batch):
    ...
    message = "You have accepted a new batch, at " + my_date.strftime("%I:%M %p") + " located in " + \
              batch['warehouse']['name'] + " of " + batch['suggested_warehouse_location']['name'] + " paid for " + batch['earnings']['estimate']
    conn = http.client.HTTPSConnection("api.sms.to")
    payload = "{\"body\": \"" + message + "\",\"to\": \"" + phone + "\",\"sender_id\": \"SMSto\",\"callback_url\": \"https://your-website.com/handle-notification\"}"
    print(payload)
